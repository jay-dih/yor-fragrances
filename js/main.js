...
function renderUserDropdown() {
  ...
  if (user) {
    content.innerHTML = `
      ...
      ${user.role === "admin" ? '<a href="admin.html">⚙️ Admin Panel</a>' : '<a href="my-orders.html" style="display:flex; align-items:center; justify-content:space-between; width:100%;"><span>📦 My Orders</span><span id="my-orders-badge" style="display:none; width:8px; height:8px; background:#ff4d4d; border-radius:50%;"></span></a>'}
      <button onclick="logoutUser()">Logout</button>
    `;

    setTimeout(() => {
      const navBadge = document.getElementById("userNavBadge");
      const ordersBadge = document.getElementById("my-orders-badge");
      if (ordersBadge && navBadge && navBadge.style.display === "block") {
...
function renderProducts(gridId, products) {
  ...
  if (!products || products.length === 0) {
    grid.innerHTML =
      '<p style="padding:40px;color:var(--text-light);grid-column:1/-1">No products found.</p>';
    return;
  }
...
async function openQuickView(id) {
  ...
  document.getElementById("qvDesc").textContent =
    p.description || "A uniquely crafted YOR original fragrance.";
  ...
  if (revRes && revRes.success && revRes.reviews.length > 0) {
    const avg =
      revRes.reviews.reduce((s, r) => s + parseInt(r.rating), 0) /
      revRes.reviews.length;
    reviewHtml =
...
function renderCart() {
  ...
  const summaryEl =
    document.getElementById("cartDrawerSummary") ||
...
function selectPayment(method) {
  ...
  document
    ...
    ?.classList.toggle("selected", method === "COD");
  document
    .getElementById("optGCash")
    ?.classList.toggle("selected", method === "GCash");
  document.getElementById("gcashFlow").style.display =
    method === "GCash" ? "block" : "none";
  document.getElementById("codFlow").style.display =
...
async function confirmPayment() {
  ...
  if (_selectedPayment === "GCash") {
    ...
    res = await fetch(API_BASE + "orders.php", { method: "POST", body: fd })
      .then((r) => r.json())
      .catch(() => null);
  } else {
...
async function handleSearch(val) {
  ...
  searchTimeout = setTimeout(async () => {
    ...
    if (results.length === 0) {
      dropdown.innerHTML =
        '<p style="padding:12px 16px;font-size:0.82rem;color:var(--text-light)">No results found</p>';
    } else {
...
async function fetchLocationSuggestions(query) {
  ...
  if (query.length < 3) return; // Only fetch suggestions for queries longer than 2 characters

  try {
    const response = await fetch(
      `api/db.php?search=${encodeURIComponent(query)}`,
    );
    ...
    suggestions.forEach((item) => {
      ...
      li.addEventListener("click", () => {
        document.getElementById("locationSearch").value = item.address;
        suggestionsList.innerHTML = "";
      });
...
async function checkCustomerNotifications() {
  ...
  try {
    const res = await apiFetch("user_orders.php?" + params.toString());
    if (res && res.success && res.orders && res.orders.length > 0) {
      const orderStates = {};
      res.orders.forEach((o) => {
        orderStates[o.id] = {
          status: o.status,
          remarks: o.admin_remarks || "",
        };
      });

      const cachedStatesStr = localStorage.getItem("customer_orders_state");
      if (cachedStatesStr) {
        const cached = JSON.parse(cachedStatesStr);
        let hasUpdates = false;

        for (const id in orderStates) {
          if (!cached[id]) {
            if (
              orderStates[id].status !== "Pending" ||
              orderStates[id].remarks
            ) {
              hasUpdates = true;
            }
          } else {
            if (
              cached[id].status !== orderStates[id].status ||
              cached[id].remarks !== orderStates[id].remarks
            ) {
              hasUpdates = true;
            }
          }
        ...
        if (hasUpdates) {
          ...
          const ordersBadge = document.getElementById("my-orders-badge");
          if (ordersBadge) ordersBadge.style.display = "block";
        }
      } else {
        localStorage.setItem(
          "customer_orders_state",
          JSON.stringify(orderStates),
        );
      }
    }
  } catch (e) {
...
document.addEventListener("DOMContentLoaded", () => {
  document
    .querySelectorAll(".fade-in, .reveal-up")
    .forEach((el) => observer.observe(el));

  // Dynamically inject user account menu notification badge
  const userMenu = document.querySelector(".nav-user-menu");
  if (userMenu) {
    userMenu.style.position = "relative";
    const badge = document.createElement("span");
    badge.id = "userNavBadge";
    badge.style.cssText =
      "display:none; position:absolute; top:-2px; right:-2px; width:7px; height:7px; background:#ff4d4d; border-radius:50%; border:1px solid white; z-index:100;";
...
// ---- REMOVE RED DOT ON MY ORDERS TAB CLICK ----
const myOrdersTab = document.querySelector(
  '#userDropdown a[href="my-orders.html"]',
);
const ordersBadge = document.getElementById("my-orders-badge");

if (myOrdersTab && ordersBadge) {
  myOrdersTab.addEventListener("click", () => {
    ordersBadge.style.display = "none";
    // Optionally, update the server to mark notifications as read
